package com.xyj.common.tool.upload.fileupload;
        ...
        try {
            ...
            for (FileItem file : fileItemList) {
                ...
                if (!file.isFormField()) {
                    ...
                    fileName = fileName.substring(fileName.lastIndexOf(fs) + 1,
                            fileName.length());
                    ...
                    if (!file.getName().trim().equals("")) {
                        ...
                        fileInfo.getFileMap().put(fileBean.getFieldName(),
                                fileBean);
                    }

                } else {
                    // 获取表单项名和表单项值 放入map中
                    paramMap.put(file.getFieldName(),
                            file.getString(fileInfo.getEncoding()));
                }
            }
        } catch (Exception e) {
